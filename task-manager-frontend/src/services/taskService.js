...
const addTask = (title, description,deadline, status) => {
    return Axios.post("http:// localhost:3001/api/tasks", { title, description,deadline, status }); // タスクを追加するためのPOSTリクエスト
};

const updateTask = (id,title, description,deadline, status) => {
  return Axios.put(`http:// localhost:3001/api/tasks/${id}`, { title, description,deadline, status }); // 指定したidのタスクを更新するためのPUTリクエスト
};
