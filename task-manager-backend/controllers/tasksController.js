...
const addTask = (req, res) => {
  ...
  const sqlInsert =
    "INSERT INTO tasks (title, description, deadline, status) VALUES (?, ?, ?, ?)"; // タスクのタイトル、説明、期限、ステータスをデータベースへ追加
...
const updateTask = (req, res) => {
  ...
  const sqlUpdate =
    "UPDATE tasks SET title = ?, description = ?, deadline = ?, status = ? WHERE id = ?"; // タスクのタイトル、説明、期限、ステータスを更新
  db.query(
    sqlUpdate,
    [title, description, deadline, status, taskId],
    (err, result) => {
